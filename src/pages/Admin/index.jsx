...
export function AdminPage({ showToast }) {
    ...
    const adminErrorMessage = (error) => {
        if (!isPermissionDenied(error)) return error?.message || "Admin request failed.";
        if (!user) return "SIGN IN FIRST, THEN ACCESS ADMIN DASHBOARD.";
        return "FIRESTORE RULES BLOCKED IN PRODUCTION. DEPLOY FIRESTORE/STORAGE RULES.";
    };
    /* ── Auth ── */
    const handleAuth = (e) => {
        e.preventDefault();
        if (!user) {
            showToast("SIGN IN REQUIRED BEFORE ADMIN ACCESS", "ng");
            return;
        }
    ...
    async function loadAnalytics() {
        try {
            ...
            setAnalytics(data);
        } catch (e) { showToast(adminErrorMessage(e), "ng"); }
    }

    async function loadQuizList() {
        try { setQuizList(await getAdminQuizQuestions()); } catch (e) { showToast(adminErrorMessage(e), "ng"); }
    }

    async function loadSimList() {
        try { setSimList(await getAdminSimScenarios()); } catch (e) { showToast(adminErrorMessage(e), "ng"); }
    }
    ...
    const handleSubmitQuiz = async (e) => {
        ...
        try {
            ...
            loadQuizList();
        } catch (err) { showToast(adminErrorMessage(err), "ng"); }
    ...
    const handleSyncQuizContent = async () => {
        ...
        try {
            ...
            showToast(adminErrorMessage(err), "ng");
        } finally {
            setLoading(false);
        }
    };
